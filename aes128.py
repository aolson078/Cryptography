...
def sbox(state):
    ...
    for i in range(4):
        for j in range(4):
            output_state.append(sboxes[int(state[i][j])])
...
def mix_column(column):
    ...
    r0 = (r00 ^ r01 ^ column[2] ^ column[3]) # r0 is the first value of the column

    r11 = galois_mult(column[1], 2)
    r12 = galois_mult(column[2], 3)
    r1 = (column[0] ^ r11 ^ r12 ^ column[3])  # r1 is the second value of the column

    r22 = galois_mult(column[2], 2)
    r23 = galois_mult(column[3], 3)
    r2 = (column[0] ^ column[1] ^ r22 ^ r23)  # r1 is the second value of the column

    r30 = galois_mult(column[0], 3)
    r33 = galois_mult(column[3], 2)
    r3 = (r30 ^ column[1] ^ column[2] ^ r33)  # r1 is the second value of the column
    return [r0, r1, r2, r3]
...
def generate_subkeys(key):
    # Define the S-box and the Rcon array
    ...
    for i in range(4, 44):
        ...
        if i % 4 == 0:
            ...
            temp = [s_box[temp[(j + 1) % 4]] for j in range(4)]  # Rotate and substitute
            temp[0] ^= rcon[i//4-1]  # XOR with round constant
